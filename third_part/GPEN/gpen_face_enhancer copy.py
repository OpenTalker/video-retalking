...
class FaceEnhancement(object):
    ...
    def process(self, img, ori_img, face_enhance=True, possion_blending=False):
        ...
        if self.use_sr and img_sr is not None:
            ...
            img = np.uint8(cv2.resize(img, (height, width)))

        else:
            
            imgg = cv2.convertScaleAbs(img*(1-full_mask) + full_img*full_mask)
            img = cv2.convertScaleAbs(ori_img*(1-mask_sharp) + imgg*mask_sharp)

        return img, orig_faces, enhanced_faces
    
    def processV2(self, img, ori_img, bbox=None, face_enhance=True, possion_blending=False):
        if self.use_sr:
            img_sr = self.srmodel.process(img)
            if img_sr is not None:
                img = cv2.resize(img, img_sr.shape[:2][::-1])
